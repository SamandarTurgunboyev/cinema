...
const TvId = ({ lang }) => {
  ...
  const getApi = useCallback(async () => {
    try {
      const res = await axios.get(`https://api.themoviedb.org/3/tv/${id}?language=en-US`, {
        headers: {
      ...
      const actor = await axios.get(`https://api.themoviedb.org/3/tv/${id}/credits?language=${lang}`, {
        headers: {
          Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiNDgzZmQzZDhlNDRlNTUwYjdiODdiNGFiNmY2NGFiMCIsInN1YiI6IjY0ZGJmN2EzMzcxMDk3MDExYzUxY2U0MyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.lbWI1cBSG2Za2GsRzMOVRqBmoOBn51Zh7l9Ga0TGe-o'
        },
      })
      const image = await axios.get(`https://api.themoviedb.org/3/tv/${id}/images`, {
        headers: {
          Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiNDgzZmQzZDhlNDRlNTUwYjdiODdiNGFiNmY2NGFiMCIsInN1YiI6IjY0ZGJmN2EzMzcxMDk3MDExYzUxY2U0MyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.lbWI1cBSG2Za2GsRzMOVRqBmoOBn51Zh7l9Ga0TGe-o'
        },
      })
            ...
            return (
              <Card sx={{ width: "100%", marginTop: "100px", paddingLeft: '20px', background: "#646464" }}>
                <CardActionArea>
                  <CardMedia
                    component="img"
                    sx={{ height: actors?.cast?.length === 1 ? "50%" : "100%", width: actors?.cast?.length === 1 ? "50%" : "100%" }}
